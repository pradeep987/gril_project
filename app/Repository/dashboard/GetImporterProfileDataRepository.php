...
class GetImporterProfileDataRepository extends Model{

	public function getImporterProfileData($userSessId)
	{
		if($userSessId > 0){
			$query = DB::table('importer_user_profile')->where("comm_seller_user_id","=",$userSessId)->get();
				
			return $query;
		}
	}

	public function saveMyImporterBussReg($data, $userSessId)
	{
		...
		if(is_numeric($id) && $id > 0){
			DB::table('importer_user_profile')->where('id', $id)->update(['title' => $coll->get('title'), 'clasification' => $coll->get('clasification'), 'description' => $coll->get('description')]);
			$returnVal = "update";
		}
	...
	public function saveMyImporterBussProf($data, $userSessId)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
		...
		if(is_numeric($id) && $id > 0){
			DB::table('importer_user_profile')->where('id', $id)->update(
	...
	public function saveImporterGoogleMapLocation($data)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
