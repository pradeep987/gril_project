...
class GetManufactureProfileDataRepository extends Model{

	public function getManufactureProfileData($userSessId)
	{
		if($userSessId > 0){
			$query = DB::table('manufacture_user_profile')->where("comm_seller_user_id","=",$userSessId)->get();
				
			return $query;
		}
	}

	public function saveMyManufactureBussReg($data, $userSessId)
	{
		...
		if(is_numeric($id) && $id > 0){
			DB::table('manufacture_user_profile')->where('id', $id)->update(['title' => $coll->get('title'), 'clasification' => $coll->get('clasification'), 'description' => $coll->get('description')]);
			$returnVal = "update";
		}
	...
	public function saveMyManufactureBussProf($data, $userSessId)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
		...
		if(is_numeric($id) && $id > 0){
			DB::table('manufacture_user_profile')->where('id', $id)->update(
	...
	public function saveManufactureGoogleMapLocation($data)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
