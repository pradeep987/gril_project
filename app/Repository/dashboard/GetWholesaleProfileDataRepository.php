...
class GetWholesaleProfileDataRepository extends Model{

	public function getWholesaleProfileData($userSessId)
	{
		if($userSessId > 0){
			$query = DB::table('whole_user_profile')->where("comm_seller_user_id","=",$userSessId)->get();
				
			return $query;
		}
	}

	public function saveMyWholesaleBussReg($data, $userSessId)
	{
		...
		if(is_numeric($id) && $id > 0){
			DB::table('whole_user_profile')->where('id', $id)->update(['title' => $coll->get('title'), 'clasification' => $coll->get('clasification'), 'description' => $coll->get('description')]);
			$returnVal = "update";
		}
	...
	public function saveMyWholesaleBussProf($data, $userSessId)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
		...
		if(is_numeric($id) && $id > 0){
			DB::table('whole_user_profile')->where('id', $id)->update(
	...
	public function saveWholesaleGoogleMapLocation($data)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
