...
class GetRentalProfileDataRepository extends Model{

	public function getRentalProfileData($userSessId)
	{
		if($userSessId > 0){
	...
	public function saveMyRentalBussReg($data, $userSessId)
	{
	...
	public function saveMyRentalBussProf($data, $userSessId)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
	...
	public function saveRentalGoogleMapLocation($data)
	{
		$data = json_decode($data, true);
		$coll = collect($data);
		$id = $coll->has('id') ? $coll->get('id') : 0;
