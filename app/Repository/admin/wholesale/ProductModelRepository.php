...
class ProductModelRepository extends Model{
	...
	{
		$productModelList = DB::table('mstr_wholesale_prod_model as pm')->select(array('pm.id', 'pm.name', 'pm.buss_cat_id as bcId', 'bc.name as bcVal', 'pm.buss_type_id as btId', 'bt.name as btVal', 'pm.prod_type_id as ptId', 'pt.name as ptVal', 'pm.seo_title', 'pm.seo_url', 'pm.seo_desc', 'pm.seo_keywords', 'pm.seo_tags', DB::raw('if(pm.active = 1, "True", "False") as active')))
			->join('mstr_wholesale_buss_category as bc', 'bc.id', '=', 'pm.buss_cat_id')
			->join('mstr_wholesale_buss_type as bt', 'bt.id', '=', 'pm.buss_type_id')
			->join('mstr_wholesale_prod_type as pt', 'pt.id', '=', 'pm.prod_type_id')->get();
	...
	public function saveItem($data)
	{
		...
		$query = DB::table('mstr_wholesale_prod_model')->where('prod_type_id', '=', $ptId);
		...
		if(is_numeric($id) && $id > 0){
			DB::table('mstr_wholesale_prod_model')->where('id', $id)->update(
			...
			$returnVal = DB::table('mstr_wholesale_prod_model')->insertGetId(
				['buss_cat_id' => $bcId, 'buss_type_id' => $btId, 'prod_type_id' => $ptId, 'buss_cat_name' => $bussCategoryName, 'buss_type_name' => $bussTypeName, 'prod_type_name' => $prodTypeName, 'name' => $productModel, 'seo_title' => $seo_title, 'seo_url' => $seo_url, 'seo_desc' => $seo_desc, 'seo_keywords' => $seo_keywords, 'seo_tags' => $seo_tags, 'active' => $active]
			);
		}
	...
	{
		$query = DB::table('mstr_wholesale_prod_model')->where('id', '=', $id)->delete();
		$id = 0;
		if($query){
			$id = 1;
		}
