...
class BusinessTypeRepository extends Model{
	...
	{
		$businessTypeList = DB::table('mstr_serviceprovider_buss_type as bt')->select(array('bt.id', 'bt.name', 'bt.buss_cat_id as bcId', 'bc.name as bcVal', 'bt.seo_title', 'bt.seo_url', 'bt.seo_desc', 'bt.seo_keywords', 'bt.seo_tags', DB::raw('if(bt.active = 1, "True", "False") as active')))
			->join('mstr_serviceprovider_buss_category as bc', 'bc.id', '=', 'bt.buss_cat_id')->get();
	...
	public function saveItem($data)
	{
		...
		$query = DB::table('mstr_serviceprovider_buss_type')->where('buss_cat_id', '=', $bcId);
		...
		if(is_numeric($id) && $id > 0){//echo "4";
			DB::table('mstr_serviceprovider_buss_type')->where('id', $id)->update(
			...
			$returnVal = "update";
			 ...
			 $returnVal = DB::table('mstr_serviceprovider_buss_type')->insertGetId(
				['buss_cat_id' => $bcId, 'name' => $businessType, 'seo_title' => $seo_title, 'seo_url' => $seo_url, 'seo_desc' => $seo_desc, 'seo_keywords' => $seo_keywords, 'seo_tags' => $seo_tags, 'active' => $active]
			);
	...
	{
		$query = DB::table('mstr_serviceprovider_buss_type')->where('id', '=', $id)->delete();
		$id = 0;
		if($query){
			$id = 1;
		}
