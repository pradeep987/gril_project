...
class BusinessCategroyRepository extends Model{
	...
	{
		$businessCategroyList = DB::table('mstr_importer_buss_category')->select(array('id', 'name', 'seo_title', 'seo_url', 'seo_desc', 'seo_keywords', 'seo_tags', DB::raw('if(active = 1, "True", "False") as active')))->get();
	...
	public function saveItem($data)
	{
		...
		$query = DB::table('mstr_importer_buss_category');
		...
		if(is_numeric($id) && $id > 0){
			DB::table('mstr_importer_buss_category')->where('id', $id)->update(
			...
			$returnVal = DB::table('mstr_importer_buss_category')->insertGetId(
				['name' => $businessCategroy, 'seo_title' => $seo_title, 'seo_url' => $seo_url, 'seo_desc' => $seo_desc, 'seo_keywords' => $seo_keywords, 'seo_tags' => $seo_tags, 'active' => $active]
			);
		}
	...
	{
		$query = DB::table('mstr_importer_buss_category')->where('id', '=', $id)->delete();
		$id = 0;
		if($query){
			$id = 1;
		}
