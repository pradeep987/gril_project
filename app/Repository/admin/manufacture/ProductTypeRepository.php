...
class ProductTypeRepository extends Model{
	...
	{
		$productTypeList = DB::table('mstr_manufacture_prod_type as pt')->select(array('pt.id', 'pt.name', 'pt.buss_cat_id as bcId', 'bc.name as bcVal', 'pt.buss_type_id as btId', 'bt.name as btVal', 'pt.seo_title', 'pt.seo_url', 'pt.seo_desc', 'pt.seo_keywords', 'pt.seo_tags', DB::raw('if(pt.active = 1, "True", "False") as active')))
			->join('mstr_manufacture_buss_category as bc', 'bc.id', '=', 'pt.buss_cat_id')
			->join('mstr_manufacture_buss_type as bt', 'bt.id', '=', 'pt.buss_type_id')->get();
	...
	public function saveItem($data)
	{
		...
		$query = DB::table('mstr_manufacture_prod_type')->where('buss_type_id', '=', $btId);
		...
		if(is_numeric($id) && $id > 0){
			DB::table('mstr_manufacture_prod_type')->where('id', $id)->update(
			...
			$returnVal = DB::table('mstr_manufacture_prod_type')->insertGetId(
				['buss_cat_id' => $bcId, 'buss_type_id' => $btId, 'name' => $productType, 'seo_title' => $seo_title, 'seo_url' => $seo_url, 'seo_desc' => $seo_desc, 'seo_keywords' => $seo_keywords, 'seo_tags' => $seo_tags, 'active' => $active]
			);
		}
	...
	{
		$query = DB::table('mstr_manufacture_prod_type')->where('id', '=', $id)->delete();
		$id = 0;
		if($query){
			$id = 1;
		}
